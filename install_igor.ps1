...
if (Get-Command ollama -ErrorAction SilentlyContinue) {
    ...
    # Ensure the server is running before we try to pull
    $ollamaRunning = $false
    try {
...
"@ | Out-File -FilePath $envFile -Encoding UTF8 -NoNewline:$false
...
# Inline signing - same logic as sign_igor_script.ps1 but uses actual $RepoRoot
# (the standalone sign script has OneDrive path hardcoded and would skip on this box)
...
# Write igor function to the current user's PS profile
$profilePath = $PROFILE.CurrentUserAllHosts   # persists across PS versions
...
# Igor launcher - added by install_igor.ps1
function igor { & "$loopScript" @args }
"@
...
if (Test-Path $skillsSrc) {
    ...
    Write-OK "Skills installed to $skillsDst (restart Claude Code to pick them up)"
} else {
    Write-Warn "cc_skills not found at $skillsSrc - skipping (git pull and re-run)"
}
