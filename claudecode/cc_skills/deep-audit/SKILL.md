...
head -60 ~/TheIgors/design_docs_for_igor/decisions_log.dsb
...
```bash
python3 ~/TheIgors/claudecode/channel.py post "deep-audit complete — <N> findings, top issue: <one line>" --as claude-code
```
...
Close ticket:
```bash
python3 ~/TheIgors/claudecode/cc_queue.py done T-deep-audit-parallel "<one paragraph summary of findings>"
```
