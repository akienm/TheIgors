...
```bash
python3 ~/TheIgors/claudecode/cc_queue.py reset <ticket-id>
```
...
```bash
python3 ~/TheIgors/claudecode/cc_queue.py worker-launch
```
...
```bash
python3 ~/TheIgors/claudecode/channel.py post "sprint-minion: pushed <ticket-id> to worker queue — daemon will pick up within 20s" --as cc
```
...
To monitor: run `/readigor` or `python3 ~/TheIgors/claudecode/channel.py read 10`
