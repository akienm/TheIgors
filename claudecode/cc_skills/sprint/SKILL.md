...
Post to channel:
```bash
python3 ~/TheIgors/claudecode/channel.py post "claimed <ticket-id>: <title>" --as <tab>
```
...
```bash
python3 ~/TheIgors/claudecode/channel.py post "progress: <what just happened>" --as <tab>
```
...
```bash
python3 ~/TheIgors/claudecode/cc_queue.py show <ticket-id> | grep -i "probe:"
```
...
```bash
python3 ~/TheIgors/claudecode/cc_queue.py done <ticket-id> "<one paragraph summary>"
```
...
```bash
python3 ~/TheIgors/claudecode/channel.py post "done: <ticket-id> — <one line summary>" --as <tab>
```

Update slate if this ticket was listed there:
```bash
IGOR_HOME_DB_URL=$DB python3 ~/TheIgors/claudecode/slate_manager.py close-ticket <ticket-id>
