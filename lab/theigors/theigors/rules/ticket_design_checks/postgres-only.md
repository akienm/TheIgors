# postgres-only — data persistence must use Postgres

**Path:** `theigors/rules/ticket_design_checks/postgres-only`
...
failure_message: |
  ...
  Postgres only, no SQLite or file-backed fallbacks. See theigors/rules/database.
