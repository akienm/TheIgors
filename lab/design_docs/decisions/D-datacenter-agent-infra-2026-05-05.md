...
- T-adc-db-proxy-wire (S) — DESIGNED: Add make_dc_proxy() to agent_datacenter; wire AGENT_DATACENTER_DB_URL  `done` — make_dc_proxy() in agent_datacenter/db.py (self-contained, no TheIgors imports per CLAUDE.md rule). AGENT_DATACENTER_DB_URL wired. 4 new tests pass. commit dc33966.
