...
**Closed at:** 2026-04-22T21:49:55.089617+00:00
**Ticket count:** 7 (all closed)

### Shipped via
- T-gist-before-retrieve (S) — Reorder preparse — graph-tree lookup BEFORE cortex.search  `done` — gist_gate.should_skip_memory_search() + main.py wire. Reflex intents (greeting) skip cortex.search when thalamus/BG gist-pass is confident (habit selected OR confidence≥0.7, tunable via IGOR_GIST_CONF
- T-non-terminal-emission (M) — Reply emission as pursuit-child — parent resumes for residue scan  `done` — residue_scan.py stub module + main.py hook after resume_parent. reply_state now threads input_text + reply_text + addressed_span. reply pursuit carries input_text in metadata. Exception-wrapped so hoo
- T-tutor-not-oracle-prompt (M) — Upstream LLM prompt shape — thinking-frame, not answer  `done` — TUTOR_DIRECTIVE constant + mode='tutor'|'answer' param on reasoning_context. Tutor is default; asks LLM for thinking-frame (questions, options, considerations) not direct answer. Answer mode preserved
- T-confidence-gated-depth-scoping (S) — Design-only — scope per-stage confidence emission + gating into concrete tickets  `done` — design doc lab/design_docs/confidence_gated_depth.md — per-stage inventory of all 9 cognitive stages + 9 proposed per-stage tickets for Akien review. Recognize stage flagged mature (T-gist-before-retr
- T-salience-residue-scan (M) — Residue scan — 'anything else interesting?' after reflex reply  `done` — residue_scan.scan_after_reply body replaces T-non-terminal-emission stub. Chunks input via T-input-chunker, classifies first chunk as reflex, scores residue via graph-tree signals (question-mark / con
- T-local-preparse-fallback (M) — Local mini-LLM fallback when graph-tree confidence is low  `done` — local_preparse.LocalPreparser class + preparse_local() convenience + default_preparser singleton. qwen2.5:0.5b default model, 1s default timeout, IGOR_LOCAL_PREPARSE_* env knobs. Returns PARSED_INPUT 
- T-shadow-stream-reasoning (L) — Shadow-stream reasoning — dual-path execution + divergence corpus  `done` — ShadowReasoner class + DivergenceRecord + reasoning_context(mode=tutor) path + Jaccard comparison + instance.reasoning_divergence schema + fire_and_forget daemon-thread logging + run_shadow_sync scaff
