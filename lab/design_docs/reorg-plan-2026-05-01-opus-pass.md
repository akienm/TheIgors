...
| **Seed scripts** (`seed_*.py` × ~80) | ~80 files | UNCLEAR — bucket decision needed | Almost all dated Apr 7. Per palace_migration shape lock 2026-04-20, most/all seeded into palace already. **Akien decide as one bucket:** archive-to-TheIgorsProject if palace migration confirmed complete, else STAY. |
...
| **`cc_memory_seed/`** (43 markdown notes + MEMORY.md + sessions.md) | 1 dir | UNCLEAR — likely MOVE → TheIgorsProject (auto_memory_archive) | Old auto-memory snapshots, Mar 23 dated; current auto-memory lives at `~/.claude/projects/-home-akien-TheIgors/memory/`. The project already has `TheIgorsProject/20260427.Cleanup/auto_memory_archive/` as the destination. |
