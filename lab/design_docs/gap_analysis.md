...
- **G-OVN-3 ~~CLOSED~~**: System crontab drain cron now uses absolute path `/home/akien/TheIgors/lab/claudecode/drain_learn_queue.py`. Issue #224. (Fixed prior session via sed+crontab.)
...
- **D089 — headless Worker script**: `claudecode/worker` bash script — `claude -p` loop, one task per invocation, logs to `~/.TheIgors/logs/worker_role.log`; launch via `nohup ~/TheIgors/lab/claudecode/worker &`. No terminal needed.
