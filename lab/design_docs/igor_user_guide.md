...
Type `/skill-name` in a Claude Code session. All skills live in `~/.claude/skills/` (deployed from `UnseenUniversity/skills/` via `agentctl skills deploy`). The TheIgors-internal skills `map-igor` and `readigor` remain in `lab/claudecode/cc_skills/` because they read Igor-specific runtime state.
...
python3 ~/TheIgors/lab/claudecode/channel.py send "CC: check your habits"
...
python3 ~/TheIgors/lab/claudecode/channel.py read 20 | grep -A3 '"author": "igor"'
```
...
IGOR_HOME_DB_URL=$DB python3 ~/TheIgors/lab/claudecode/session_manager.py start "2026-03-26a" "Theme: one line"

# Record a decision
IGOR_HOME_DB_URL=$DB python3 ~/TheIgors/lab/claudecode/decision_manager.py add D123 "short-name" "status" "one-line description"

# Finalize session
IGOR_HOME_DB_URL=$DB python3 ~/TheIgors/lab/claudecode/session_manager.py finalize "2026-03-26a" "Next: ..." "In-flight: ..."

# Flush summary to Igor memory
python3 ~/TheIgors/lab/claudecode/cc_queue.py flush_session 2026-03-26a "theme: ...; next: ..."
```
...
EOF
python3 ~/TheIgors/lab/claudecode/cc_queue.py add /tmp/task.json

# List tasks
python3 ~/TheIgors/lab/claudecode/cc_queue.py list

# Mark done
python3 ~/TheIgors/lab/claudecode/cc_queue.py done <task-id> "what was built"
```
...
IGOR_HOME_DB_URL=$DB python3 ~/TheIgors/lab/claudecode/seed_self_training_habit.py
IGOR_HOME_DB_URL=$DB python3 ~/TheIgors/lab/claudecode/seed_output_training_habit.py
IGOR_HOME_DB_URL=$DB python3 ~/TheIgors/lab/claudecode/seed_routing_habits.py
...
python3 ~/TheIgors/lab/claudecode/book_learner.py --calibre-id <id> --title "Book Title"

# Drain the reading queue (runs book_learner in sequence)
python3 ~/TheIgors/lab/claudecode/drain_learn_queue.py

# Launch overnight reading session
python3 ~/TheIgors/lab/claudecode/launch_overnight_reading.py
```
...
```bash
# General diagnostics
python3 ~/TheIgors/lab/claudecode/diag.py

# Eval preparse (test thalamus parsing on an input)
python3 ~/TheIgors/lab/claudecode/eval_preparse.py "your test input"

# CC deposit (deposit a memory directly)
python3 ~/TheIgors/lab/claudecode/cc_deposit.py
```
