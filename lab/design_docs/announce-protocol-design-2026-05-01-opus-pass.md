...
`UnseenUniversity/devices/claude/announce_mcp.py` for CC. This honors the
...
|---|---|---|
| Per-device descriptor | `UnseenUniversity/device.py` lines 17-97 | `BaseDevice.who_am_i() / capabilities() / comms() / requirements()` already returns the data the manifest needs to project |
| Device registry | `skeleton/registry.py` lines 38-115 | Flat-file `~/.unseen_university/devices.json`, atomic-write, online/offline/blocked status |
| Skeleton (rack + MCP aggregator) | `UnseenUniversity/skeleton/skeleton.py` | Already registers per-device MCP tools dynamically (lines 148-203). Already exposes `rack.devices` / `rack.health` / `rack.channels` MCP tools |
...
| Router | `UnseenUniversity/bus/router.py` lines 39-114 | Resolves `comms://<mailbox>` → IMAP `APPEND`, with self-healing relaunch |
...
```python
# UnseenUniversity/announce/envelope.py
...
|---|---|---|---|---|
| **Profile** | `~/.unseen_university/profiles/<agent_id>.yaml` | Humans (committed under `UnseenUniversity/config/profiles/` and synced to the runtime dir on install) | Days–months | Agent **type** (igor, cc, research-*) |
...
`~/.unseen_university/profiles/` solves it; atomic-write keeps it safe
...
```yaml
# ~/.unseen_university/profiles/igor.yaml
profile_version: "1.0"
agent_type: igor
...
```yaml
# ~/.unseen_university/profiles/cc.yaml
profile_version: "1.0"
agent_type: cc
...
```yaml
# ~/.unseen_university/profiles/research-orca.yaml
profile_version: "1.0"
...
```python
# UnseenUniversity/announce/profile.py
from pathlib import Path
import yaml

PROFILES_DIR = Path("~/.unseen_university/profiles").expanduser()
...
```python
# UnseenUniversity/announce/manifest.py
from dataclasses import dataclass, field
...
| Profile YAML edited on disk | `inotify` watch on `~/.unseen_university/profiles/` | Push to agents of the changed type only |
| ACL change | (future) ACL admin tool | Push to affected agents |
| Registry file edited externally | `inotify` on `~/.unseen_university/devices.json` | Push to all agents (registry_etag changes) |
...
### 10.1 Verifying the intuition: yes, an MCP server in `UnseenUniversity/devices/claude/`

**Confirmed.** The right pattern is:

- **New device:** `UnseenUniversity/devices/claude/announce_mcp.py` — an
...
- **MCP config:** `~/TheIgors/.mcp.json` (already exists; current shape
  ...
  ```json
  {
    "mcpServers": {
      "datacenter": {
        "command": "/home/akien/dev/src/UnseenUniversity/.venv/bin/python",
        "args": ["-m", "devices.claude.announce_mcp"],
        "env": {
          "CLAUDE_SESSION_ID": "${CLAUDE_SESSION_ID}",
          "UNSEEN_UNIVERSITY_HOME": "${HOME}/.unseen_university"
        }
      },
      "igor": { "...": "..." }
    }
...
```python
# UnseenUniversity/devices/claude/announce_mcp.py
"""
...
- Option A: hardcoded constants in `UnseenUniversity` package — every
  agent imports.
- Option B: env vars `AGENT_DATACENTER_ANNOUNCE` /
  `AGENT_DATACENTER_IMAP_URL` set by the rack on launch.
- Option C: `~/.unseen_university/bootstrap.json` — small flat file the
...
### J. Profile location: `UnseenUniversity/config/profiles/` (in repo) or `~/.unseen_university/profiles/` (runtime)?
...
- **Recommendation:** both, with sync-on-install. Repo is canonical;
  ...
  `unseen_university_home()` pattern).
- **Question for Akien:** confirmed?
...
### 12.A `tests/test_announce_protocol.py` (UnseenUniversity side)
...
- `UnseenUniversity/UnseenUniversity/announce/__init__.py`
- `UnseenUniversity/UnseenUniversity/announce/envelope.py` — IdentityEnvelope dataclass
- `UnseenUniversity/UnseenUniversity/announce/manifest.py` — Manifest + ToolBinding + ChannelSubscription + StateRef + ACL dataclasses
- `UnseenUniversity/UnseenUniversity/announce/profile.py` — load_profile, merge for inheritance
- `UnseenUniversity/UnseenUniversity/announce/broker.py` — AnnounceBroker (subdevice of skeleton); ManifestAssembler
- `UnseenUniversity/UnseenUniversity/announce/invalidator.py` — inotify watch + push-on-change
- `UnseenUniversity/config/profiles/igor.yaml`
- `UnseenUniversity/config/profiles/cc.yaml`
- `UnseenUniversity/config/profiles/research-orca.yaml` (example)
- `UnseenUniversity/devices/claude/announce_mcp.py` — CC-side MCP adapter
- `wild_igor/igor/datacenter_client.py` — Igor-side consumer
- `~/.unseen_university/bootstrap.json` — written by rack at boot, read by all agents
- `~/.unseen_university/aliases.json` — lineage alias table

**Touched files:**

- `UnseenUniversity/UnseenUniversity/skeleton/skeleton.py` — register AnnounceBroker as a sub-device; add `rack.announce_request` MCP tool for debugging
- `UnseenUniversity/UnseenUniversity/bus/router.py` — alias-table consult after primary lookup
- `UnseenUniversity/bus/imap_server.py` — ensure mailbox `announce` and `announce-events` exist at boot
...
1. **Profile (static, YAML, runtime dir) and Manifest (dynamic, derived,
   ...
   `~/.unseen_university/profiles/<agent_id>.yaml` (committed under
   `UnseenUniversity/config/profiles/`, synced on install). Manifest is
...
4. **Lineage form (`comms://igor-wild-0001`) is aliased, not replaced,
   in v1.** Alias table at `~/.unseen_university/aliases.json`.
...
7. **CC plugs in via a new MCP server**
   (`UnseenUniversity/devices/claude/announce_mcp.py`) registered in
...
| 11.J — profile location | **Both — repo canonical, runtime read** | Opus rec accepted. Sync-on-install. Matches existing `unseen_university_home()` pattern. |
...
- G-decision shifts how bootstrap is implemented: Igor needs a shim added to `UnseenUniversity/devices/igor/shim.py` (likely already partially exists — verify) that carries the install + connect + capability-reading methods.
