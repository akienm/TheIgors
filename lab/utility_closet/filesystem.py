"""
filesystem.py — UC canonical home for sandboxed fs + system-inspection tools
(T-uc-filesystem-shelf).
...
PDF helpers:
  read_pdf_pages — paged PDF reading

Self-awareness tools:
  check_disk_usage, check_resource_load — disk + CPU/RAM/swap snapshot
  evaluate_threshold_habits — runs threshold-type habits against live load

## Layer policy

Canonical implementation lives here in UC. `wild_igor/igor/tools/filesystem.py`
is a thin re-export shim — existing `from ..tools.filesystem import …`
imports keep working.

Cross-layer wild_igor imports are lazy (`paths()`, `system_proxy`) so UC
never triggers wild_igor package load at import time.
"""
