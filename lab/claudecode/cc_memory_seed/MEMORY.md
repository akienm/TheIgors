...
- Queue: `~/.TheIgors/cc_channel/queue.json`; CLI: `python3 ~/TheIgors/lab/claudecode/cc_queue.py list`
...
- Igor's `read_file` tool rooted at `/home/akien` — correct path: `TheIgors/lab/design_docs/filename.txt`
...
- CC wrapper: Igor must call `~/TheIgors/lab/claudecode/cc.sh` (sets REAL_ANTHROPIC_API_KEY); not `claude` directly
