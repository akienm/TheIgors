---
name: sprint-ticket
description: Single-ticket execution unit — capability check, confirm in_progress, build, test, commit, close, savestate. Called by /sprint and /sprint-batch. Args: ticket ID.
model: sonnet
---

# /sprint-ticket — Single-ticket sprint

The atomic sprint unit. Takes a ticket ID, runs it to close,
...
Output shape (one line, before Step 2):
```
CAPABILITY CHECK: <tag/match> matches <capability> — delegate via:
  <one-line command>
Proceed inline anyway? (y to continue CC-side, or fire the delegate)
```

When no match: silent — proceed to Step 2 directly.

### 2. Confirm ticket is in_progress (dispatch IS assignment — no claim step)

The queue is an encapsulated black box. Workers request work; the act of
dispatch IS the assignment. There is no separate claim step.

Check current status:
```bash
python3 ${CC_WORKFLOW_TOOLS}/cc_queue.py show <id> | python3 -c "import sys,json; d=json.load(sys.stdin); print(d['status'])"
```

- **status = in_progress** → dispatched via cmd_next; proceed.
- **status = sprint** → direct CC sprint (Akien assigned this ticket); mark in_progress now:
  ```bash
  python3 ${CC_WORKFLOW_TOOLS}/cc_queue.py setstatus <id> in_progress
  ```
- **any other status** → surface to Akien; do not proceed.

Then add the ticket ID to today's slate under `## Planned` or `## Ad hoc`.
...
When the commit includes changes unrelated to the active ticket, always
draft a new ticket and immediately close it for the incidental fix — every
change has a ticket.

### 12.5. Levers doc check

When this sprint changed a skill file (anything under `~/.claude/skills/`) or
added a new capability to the workflow, check whether the levers doc needs
updating:

```bash
git diff --name-only HEAD~1 HEAD | grep -E "\.claude/skills/|lab/claudecode/"
```

If skill files changed: open `lab/docs/Deterministic_AI_Development_Levers_A_layer_on_top_of_skills.md`
and add or update the entry that reflects the new behavior. The levers doc is
human-authored — one paragraph per lever, no auto-generation. If the sprint
added a new lever, add a paragraph. If it changed an existing lever, update it.

When no skill files changed: skip silently.
