...
/question promote Q-xxx  — convert to hypothesis or flag for /sorted
...
Ask: "Promote to hypothesis (ready to test) or flag for /sorted (ready to design)?"

- **hypothesis**: surface the question text as the starting point for hypothesis extraction. Suggest running `/sorted` with the question as framing.
- **sorted**: mark the question as `ready_to_decide` and flag it in the next /sorted scope boundary.
