...
**Planning artifacts:** When /decided produces a spec document, comparison, or exploration doc
(beyond the decision .md stub), save it as HTML rather than Markdown. HTML enables tab navigation,
side-by-side comparisons, and richer information density without ASCII workarounds. The decision
stub at `lab/design_docs/decisions/D-...md` stays plain text (append-only log); only _supplemental_
artifacts (spec templates, option comparisons, visual guides) switch to HTML. Reference template:
`lab/docs/html_spec_template.html`.
