...
Check for collision:
```bash
python3 ~/TheIgors/lab/claudecode/cc_queue.py list 2>/dev/null | grep "<proposed-id>"
```
...
```bash
python3 ~/TheIgors/lab/claudecode/cc_queue.py add \
  --id "<id>" \
  --title "<title>" \
...
```bash
python3 ~/TheIgors/lab/claudecode/channel.py post "queued <id>: <title>" --as tab
```
