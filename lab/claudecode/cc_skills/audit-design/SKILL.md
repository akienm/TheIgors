...
### Check 10 — Encapsulation surface

**Look for:** does this design have any subsystem reaching across an interface
it shouldn't? Patterns that warrant AMEND:
- Component A knows B's internal state or selection logic when a service
  interface (dispatcher, proxy, queue) would suffice
- Shared mutable state accessed by multiple callers without a proxy layer
- Caller selecting from a list when a dispatcher could hand it the right item
  (the "Igor picking from the queue" anti-pattern)

**Advisory note (not AMEND)** when an existing coupling is named and accepted
with a reason — acknowledged debt is better than silent debt.

**Fail when:** the design introduces new cross-cutting reach that could instead
be expressed as a named service, and no exemption reason is given.

**AMEND:** "Design introduces [describe the reach] — could this be a named
service/dispatcher so callers don't need to know [describe the internals]?
See `theigors/decisions/D-queue-encapsulation-2026-05-21` for the pattern.
If coupling is intentional, name it explicitly."

Finding tag: `[encapsulation]`
...
### 3. Run all ten checks

Each check returns PASS or AMEND-with-message. Stop-on-first-fail is NOT
enabled — run all ten even if early checks fail, so the AMEND output is
...
- Always run all ten checks; don't stop on first fail.
