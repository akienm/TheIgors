...
### 9.5. Prune operational logs

Delete emission files older than 30 days — these are rolling operational
logs, not durable knowledge. Each device prunes its own local emissions.

```bash
python3 -c "
import sys; sys.path.insert(0, '${THEIGORS_HOME:-\$HOME/TheIgors}')
from lab.claudecode.emit import prune_emissions
n = prune_emissions(days=30)
print(f'emissions pruned: {n} files removed')
"
```
...
**Discussion format note:** The Discussion body uses GitHub-flavored Markdown (the GraphQL API
accepts it). For rich supplemental artifacts (capability comparisons, architecture diagrams,
option tables), save them as HTML files in `lab/docs/` and link from the Discussion body rather
than trying to embed complex layouts in Markdown. The Discussion body itself stays Markdown.
