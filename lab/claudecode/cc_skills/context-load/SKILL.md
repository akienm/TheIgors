...
D304: slates are daily files at `~/.TheIgors/lab/claudecode/YYYYMMDD.slate.txt`.
...
```bash
tail -10 ~/TheIgors/lab/design_docs_for_igor/decisions_log.dsb | sed 's/|/ — /g; s/^ *//'
```
...
```bash
DB=postgresql://igor:choose_a_password@127.0.0.1/Igor-wild-0001
python3 ~/TheIgors/lab/claudecode/session_manager.py current
IGOR_HOME_DB_URL=$DB python3 ~/TheIgors/lab/claudecode/session_manager.py show 1
```
...
```bash
IGOR_HOME_DB_URL=$DB python3 ~/TheIgors/lab/claudecode/session_manager.py start "YYYY-MM-DDx" "Theme: one line"
```
