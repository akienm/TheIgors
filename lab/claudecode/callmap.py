...
Wire as audit check:
  audit_add.py add forever callmap-fresh \
      --kind shell --pattern 'python3 lab/claudecode/callmap.py --check' \
      --severity med
...
@dataclass
class ApiEntry:
    module: str  # dotted module path, e.g. "lab.claudecode.cc_queue"
    ...
    if args.check:
        ...
        if existing != rendered:
            print(
                ...
                f"Re-run `python3 lab/claudecode/callmap.py` and commit.",
                file=sys.stderr,
            )
            return 1
